...
import Spine_navigation_Polyu.GUI.Scolioscan_robotics_big_GUI_Qt5 as GUI
import sys
import time
...
class Force_Thread(Thread):
    ...
    def run(self):
        ...
        while self.force_tread_stopper == False:
            ...
            '''prevent force to be too high, not to create big velocities'''
            ...
            '''if you want to use the position control, here is the next coordinate'''
            posa = robot.getl()
            point_x = posa[0]
            Rx_current = posa[3]
            print("robot pose X during force measurement", point_x)
            self.point_x_next = point_x + config.FORCE.K_delta * (math.fabs(self.Fz_sat) - Fref)

            ################ velocity control #################

            '''For velocity control use: '''
            self.velocity_x = Kf*(-Fref - self.Fz_sat)
            #self.velocity_x = Kf * (F - Fref)
            print("Velocity X from force thread",self.velocity_x)
...
########################____X____#################
                ...
                #TODO: switch to use the differnce of force to add increment, not giving absolute force (X = x+dX)
########################____Y____#################
...
########################____FEED to ROBOT____#################
    ...
    def move_first_point(self):
        ...
        first_pose[0][0] += 0.002 #have to give some increment, otherwise robot refuses to start from the very same position second time
