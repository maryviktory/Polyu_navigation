...
if FCN:
    ...
    ws.connect(config.IMAGE.LOCAL_HOST)
    ws.send(json_mylist)

    for i in range(3):
        binAnswer = ws.recv_frame()
        if i == 2 & websocket.ABNF.OPCODE_MAP[binAnswer.opcode] == "binary":
            logger.info("US probe is connected")
        else:
            logger.info("Probe is not connected. Please relaunch the app")
...
class Window(QtWidgets.QDialog, GUI.Ui_Dialog):
    ...
    def __init__(self):
        ...
        # if config.IMAGE.FCN == True:
            # self.FCN_thread = FCN_Thread()
            # self.stop_threads_thread = Stop_Threads()
            # self.get_image_thread = Get_Image()
...
# alignment of Window
    ...
    def stop_robot_move(self):
        ...
        self.Force_Thread.force_tread_stopper = True
        self.Position_set.stop_flag_for_robot_move = True
    ...
    def start_on_click(self):
        ...
        #TODO: check that the second loop is not blocking the other executions
        # if config.IMAGE.FCN == True:
...
#TODO: change this thread to a if else condition, when robot reaches the final point.
#TODO:  No need to follow the trajectory any more with autonomos approach
    def __init__(self,):
        Thread.__init__(self)
    ...
    def run(self):
        ...
        while self.stop_flag_for_robot_move == False:
            while i <len(self.position_from_file)-1:
                ...
                if self.stop_flag_for_robot_move == True:
                    time_thread = time.time() - time_start
                    fps_im = self.num / time_thread
                    print("fps Set Position thread {}, average time per cycle {}".format(fps_im, 1 / fps_im))
                    break
                self.num = self.num + 1
                count += 1
    ...
    def move_first_point(self):
        ...
        while St == True:
            ...
            if math.fabs(Fz_1) < config.FORCE.Fref_first_move:
                ...
                first_pose[0][force_slot] += -0.002*T_force
...
class Plot_Thread(Thread):
    def __init__(self, Pose_thread, Force_thread, Move_thread):
        ...
        self.num = 0
        self.plot_thread_stopper = False
    def run(self):
        time_start = time.time()
        while True:
            if self.radio_button_enabled == True:
                if self.move_thread.start_plotting == True:
                    ...
                    while self.plot_thread_stopper==True:
                        cp = robot.getl()
                        ...
                        if self.move_thread.plotme == True:
                            ...
                            time_thread = time.time() - time_start
                            fps_im = self.num / time_thread
                            print("fps Plot thread {}, average time per cycle {}".format(fps_im, 1 / fps_im))
