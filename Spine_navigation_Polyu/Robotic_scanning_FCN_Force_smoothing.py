...
from Spine_navigation_Polyu.utils.functions import save_video, save_video_original,save_video_filtered_points
...
if config.MODE.BASE_csys == True:
    ...
    V_control = np.zeros(3)
    axis_up_base = 2 #z in base coordinate sys
    axis_up = 1  # y tool
    axis_force = force_slot  # z tool
    axis_image = 0  # x tool

# TODO: write everything from one experiment to csv file

csv_file_name = ('results/%sPosition_plot_Fref_%s_Kf_%s_Kz_%s_thr_%s.csv' % (
config.Trajectory_n, config.FORCE.Fref, config.FORCE.Kf, config.FORCE.Kz, config.FORCE.thr))
...
class Window(QtWidgets.QDialog, GUI.Ui_Dialog):
    ...
    def __init__(self):
        ...
        if config.MODE.FCN == True:
            ...
            self.FCN_thread = FCN_Thread(self.q_im, self.threads_stopper, self.q_im_inputs, self.q_im_pred,
                                         self.q_frame_probability, self.q_num, self.v_num, self.start_send_image)
    ...
    def start_on_click(self):
        ...
        if self.lineEdit.text() != '':
            ...
            if self.last_record_point is None:
                ...
                self.move_end_distance.value = self.last_record_point[axis_up_base] - self.first_record_point_var[
                    axis_up_base]  # axis_up = Y
        ...
        self.Move_Thread = Move_Thread(self.q_Full_Force, self.q_Force, self.move_end_distance,
                                       ...
                                       self.q_im_pred, self.q_frame_probability, self.q_num, self.v_num,
...
class Move_Thread(Process):
    def __init__(self, q_Full_Force, q_Force, move_end_distance, first_record_point, threads_stopper, q_im_inputs,
                 q_im_pred, q_frame_probability, q_num, v_num, start_send_image):
        Process.__init__(self)
    ...
    def run(self):
        ...
        X_im_array = np.zeros(0)
        # X_im_array = np.append(X_im_array,640/2)

        Y_im_array = np.zeros(0)
        # Y_im_array = np.append(Y_im_array,480/2)
        X = Y = 224/2

        if config.MODE.FCN == True:
            pd_frame = pd.DataFrame(
                columns=["timestamp", "X_im", "Y_im", "Frame_Probability", "X_tcp", "Y_tcp", "Z_tcp", "X", "Y", "Z",
                         "Rx", "Ry", "Rz", "Fx", "Fy", "Fz", "Mx", "My", "Mz"])
        else:
            pd_frame = pd.DataFrame(
        ...
        while bool(self.threads_stopper.value) == False:
            ...
            pos_y_curr = T_points_intcp[1, 3] #TCP
            ...
            if config.MODE.FCN == True:
                ...
                # NOTE: if probability is higher than the threshold -> change the target position, otherwise keep previous.
                ...
                X_im_array = np.append(X_im_array, x_scaled)
                Y_im_array = np.append(Y_im_array,y_scaled)
                ...
                # NOTE: SMoohing of image predicted coordinated
                if config.MODE.median_filter == True:
                    kernelsize = config.Median_kernel_size
                    if len(X_im_array)>kernelsize:

                        X_filtered = medfilt(X_im_array,kernelsize)
                        Y_filtered = medfilt(Y_im_array,kernelsize)
                        y_scaled_filt = Y_filtered[-1]
                        x_scaled_filt = X_filtered[-1]
                        delta_X = x_scaled_filt - config.IMAGE.ORIGINAL_IMAGE_SIZE / 2  # NOTE: middle of the image, because we want to maintain feature in the middle

                        # print("X_im_array", X_im_array)
                        # print("X_filter",X_filtered,  X_filtered[-1])
                ...
                if config.MODE.exp_smoothing_velocity == True:
                    ...
                    # vel_im = -K_im * delta_X_meters * T_tool_image_x + (1 - K_im) * V_im_array[-1]
                ...
                pd_frame = pd_frame.append({'timestamp': time.time(), "X_im": x_scaled, "Y_im": y_scaled,
                                            "Frame_Probability": frame_probability, "X_tcp": T_points_intcp[0, 3],
                                            "Y_tcp": T_points_intcp[1, 3], "Z_tcp": T_points_intcp[2, 3],
                                            'X': pos_curr[0], 'Y': pos_curr[1], 'Z': pos_curr[2], 'Rx': pos_curr[3],
                                            'Ry': pos_curr[4], 'Rz': pos_curr[5], "Fx": force_curr_full[0],
                                            "Fy": force_curr_full[1], "Fz": force_curr_full[2],
                                            "Mx": force_curr_full[3], "My": force_curr_full[4],
                                            "Mz": force_curr_full[5]}, ignore_index=True)

                if config.IMAGE.VIDEO == True:
                    if config.MODE.median_filter == True :
                        self.result_im = save_video_filtered_points(self.out, inputs, pred, frame_probability, x_scaled_filt, y_scaled_filt,self.patient, config,
                                                    target=None,
                                                    labels=None)
                    else:
                        self.result_im = save_video(self.out, inputs, pred, frame_probability, self.patient, config,
                                                target=None,
                                                labels=None)
            ...
            self.num = self.num + 1
            # print("move end distance",self.move_end_distance.value)
            if math.fabs(math.fabs(pos_y_curr) - math.fabs(start_movement_Y)) > self.move_end_distance.value:
            ...
            if bool(self.threads_stopper.value) == False:
                ...
                if math.fabs(Fz_1) < config.FORCE.Fref_first_move:
                    ...
                    if config.MODE.BASE_csys == True:
                        ...
                        self.robot.movel_tool(first_pose, acc=config.FORCE.a, vel=config.FORCE.v, wait=False,
                                              relative=False,
                                              threshold=None)
                    else:
                        self.robot.z_t += 0.002 * T_force
