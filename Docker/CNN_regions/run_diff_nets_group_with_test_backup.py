...
def make_weights_for_balanced_classes_images(images, nclasses):
    count = [0] * nclasses
    for item in images:
        count[item[1]] += 1
    weight_per_class = [0.] * nclasses
...
if __name__ == '__main__':
    ...
    ####____CLASS BALANCE_____#######
    image_list = []
    class_id_list = []
    class_num = len(os.listdir(os.path.join(data_dir, "train")))
    for class_id, (class_folder) in enumerate(os.listdir(os.path.join(data_dir, "train"))):
        im_list = os.listdir(os.path.join(data_dir, "train", class_folder))
        im_list = [os.path.join(data_dir, "train", class_folder, item) for item in im_list if ".png" in item]
        class_id_list_one_folder = [class_id for item in im_list if ".png" in item]
        image_list.extend(im_list)
        class_id_list.extend(class_id_list_one_folder)

    target_list = torch.tensor(class_id_list)
    ...
    # weights_classes = make_weights_for_balanced_classes(os.path.join(data_dir, "train"), class_num)
    weights_classes = make_weights_for_balanced_classes(os.path.join(data_dir, "train"), len(image_datasets['train'].classes))
    ...
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=args.batch_size,
                                                num_workers=4, sampler=weighted_sampler)
                   for x in ['train', 'val']}
