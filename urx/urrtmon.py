...
class URRTMonitor(threading.Thread):
    ...
    def run(self):
        ...
        while not self._stop_event:
            self.__recv_rt_data()
        self._rtSock.close()
        print("connected to rt")

####____Custom functions_ Maria Victorova____#####

    def send_program_rtmon(self, prog):
        """
        send program to robot in URRobot format - port 30003
        If another program is send while a program is running the first program is aborded.
        """

        data = self.AddStatusBit2Prog(prog)

        self._rtSock.send(data.encode())

        # time.sleep(0.0001) #can change to get higher fps.



    def AddStatusBit2Prog(self, prg):
        '''
        ...
        if def1 >= 0:
            ...
            if len(prg) == prglen:
                print('Send_program: Syntax error in program')
                return False
